...
type ResponseData = { requestedDate: string; generatedAt: string; calculatorVersion: string; record: RecordData };

const number = (value: unknown, digits = 2) => typeof value === "number" && Number.isFinite(value) ? value.toFixed(digits) : "-";

export default function MarketAnalysisPanel({ code, investorData }: { code: string; investorData?: { foreignNetBuyQty?: number; institutionNetBuyQty?: number; totalNetBuyQty?: number } | null }) {
  const [data, setData] = useState<ResponseData | null>(null); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(true);
  useEffect(() => {
    const controller = new AbortController();
    fetch(`/api/market-analysis?code=${encodeURIComponent(code)}`, { signal: controller.signal, cache: "no-store" })
      .then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body?.error?.message ?? "저장된 시장분석 결과를 읽지 못했습니다."); return body; })
      .then(setData).catch((reason) => { if (reason?.name !== "AbortError") setError(reason instanceof Error ? reason.message : String(reason)); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
