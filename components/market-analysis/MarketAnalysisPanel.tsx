...
export default function MarketAnalysisPanel({ data, intraday, investorData, loading, error, source, intradayError }: { data: MarketAnalysisResponse | null; intraday: IntradayAnalysisResponse | null; investorData?: { foreignNetBuyQty?: number; institutionNetBuyQty?: number; totalNetBuyQty?: number } | null; loading: boolean; error: string | null; source?: string | null; intradayError?: string | null }) {
  ...
  return <><div className="mt-6 rounded-2xl border border-gray-200 bg-white p-5">
    ...
    <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-500"><span>기준일 {record.asOfDate}</span><span>공식 종가 {record.officialClosePrice?.toLocaleString()}원</span><span>{data.calculatorVersion} · 검증 진행 중</span><span>품질 {record.qualityStatus}</span><span>{source === "storedOfficialSnapshot" ? "저장된 공식 snapshot" : "검색 공식 일봉 계산"}</span><span>realtime 미적용</span></div>
  ...
  </div><div className="mt-4 rounded-2xl border border-indigo-200 bg-indigo-50 p-5"><p className="text-sm text-indigo-700">장중 참고 분석 · displayOnly</p>{!intraday||intraday.intradayAnalysis.blockingReasons.length?<><h3 className="mt-1 font-bold text-indigo-900">장중 시장상태 이용 불가</h3><p className="mt-2 text-sm text-indigo-700">{intradayError ?? analysisAvailabilityMessage(intraday?.intradayAnalysis.blockingReasons[0], "현재 장중 분석을 계산할 수 없습니다.")}</p></>:<><h3 className="mt-1 font-bold text-indigo-900">{intraday.session?.displayLabel}</h3><div className="mt-2 text-3xl font-bold text-indigo-900">{intraday.intradayAnalysis.finalTechnicalScore} / 100</div><p className="mt-2 text-sm">공식 점수 대비 {intraday.intradayAnalysis.scoreDifference}</p><p className="mt-2 text-xs">KIS 마지막 체결 {intraday.quote?.asOfDate} {intraday.quote?.asOfTime} · 서버 수신 {intraday.quote?.receivedAt} · 최대 60초 신선도 정책</p></>}<p className="mt-3 text-xs text-indigo-700">참고용 · TOP/백테스트/최적화 미사용</p></div></>;
}
