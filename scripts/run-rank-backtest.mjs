...
const executionPolicy=String(args["execution-policy"]??"public-eod-t2-open-v1");
if(!["public-eod-t2-open-v1","legacy-t1-open-v1"].includes(executionPolicy))throw new Error("지원하지 않는 --execution-policy입니다.");
if(executionPolicy==="legacy-t1-open-v1")console.warn("legacy-t1-open-v1: timingUnverified=true, eligibleForExecutableConclusion=false");
...
const result=buildRankBacktest({snapshots,models,evaluations,config,includeProvisional:args["include-provisional"]===true||config.includeProvisionalByDefault,generatedAt,executionPolicy});
