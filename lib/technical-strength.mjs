...
export function createTechnicalStrengthCloseSeries(priceHistory, realtimePrice = null) {
  const historicalCloses = priceHistory.map((item) => Number(item.clpr));
  const realtimeClose = Number(realtimePrice?.price);
  if (!Number.isFinite(realtimeClose) || realtimeClose <= 0) return historicalCloses;

  const realtimeDate = compactDate(realtimePrice?.asOfDate);
  const latestHistoricalDate = compactDate(String(priceHistory[0]?.basDt ?? ""));
  if (realtimeDate === null) throw new Error("realtimePrice.asOfDate가 필요합니다.");
  if (latestHistoricalDate === null) throw new Error("최신 공식 일봉 basDt가 필요합니다.");
  if (realtimeDate < latestHistoricalDate) throw new Error("realtimePrice가 최신 공식 일봉보다 오래되었습니다.");
  if (realtimeDate !== null && realtimeDate === latestHistoricalDate) {
    return [realtimeClose, ...historicalCloses.slice(1)];
  }
  return [realtimeClose, ...historicalCloses];
}

export function calculateTechnicalStrength(priceHistory, realtimePrice = null) {
  const liveCloses = createTechnicalStrengthCloseSeries(priceHistory, realtimePrice);
  const liveCurrentPrice = liveCloses[0] ?? null;
  const liveCurrentVolume = realtimePrice?.volume ?? (priceHistory.length > 0 ? Number(priceHistory[0].trqu) : null);
  const livePrices = liveCloses;
  ...
  const closes = [...liveCloses].reverse();
  ...
  const momentum20 = liveCloses.length >= 20
    ? ((liveCloses[0] - liveCloses[19]) / liveCloses[19]) * 100
    : null;
  const momentum5 = liveCloses.length >= 5
    ? ((liveCloses[0] - liveCloses[4]) / liveCloses[4]) * 100
    : null;
  const dailyReturn = liveCloses.length >= 2
    ? ((liveCloses[0] - liveCloses[1]) / liveCloses[1]) * 100
    : null;
  const recent3DailyReturns = liveCloses.length >= 4
    ? [
        ((liveCloses[0] - liveCloses[1]) / liveCloses[1]) * 100,
        ((liveCloses[1] - liveCloses[2]) / liveCloses[2]) * 100,
        ((liveCloses[2] - liveCloses[3]) / liveCloses[3]) * 100,
      ]
    : [];
  const hadRecentSurge = recent3DailyReturns.some((value) => value > 15);
  const momentum60 = liveCloses.length >= 60
    ? ((liveCloses[0] - liveCloses[59]) / liveCloses[59]) * 100
    : null;
  ...
  const currentPrice = liveCurrentPrice;
  let trendScore = 0;
