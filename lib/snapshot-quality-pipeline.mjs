...
export const QUALITY_EXCLUSION_POLICY_VERSION = "quality-quarantine-v1";
...
export function buildUniverseSummary(quality, activeComparisonModels) {
  ...
  const quarantined = buildQuarantineExclusions(quality);
  const quarantinedCodes = new Set(quarantined.map((entry) => entry.code));
  const quarantinedCodeList = [...quarantinedCodes].sort();
  const qualityEligibleCodes = observedCodes.filter((code) => !quarantinedCodes.has(code));
  ...
  return {
    ...
    originalUniverse: { count: observedCodes.length, codesHash: sha256Canonical(observedCodes) },
    qualityEligibleUniverse: { count: qualityEligibleCodes.length, codesHash: sha256Canonical(qualityEligibleCodes) },
    quarantinedUniverse: { count: quarantinedCodeList.length, codesHash: sha256Canonical(quarantinedCodeList), exclusions: quarantined },
    exclusionPolicyVersion: QUALITY_EXCLUSION_POLICY_VERSION,
    isPartialRanking: quarantinedCodeList.length > 0,
    modelEligibleUniverse,
    rankingUniverse: modelEligibleUniverse,
...
export function buildQuarantineExclusions(quality) {
  return (quality?.issues ?? [])
    .filter(isQuarantinableMarketDataIssue)
    .map((entry) => ({ code: entry.code, reason: entry.type, issueDate: entry.date ?? null, disposition: "quarantine" }))
    .sort((left, right) => left.code.localeCompare(right.code) || String(left.issueDate).localeCompare(String(right.issueDate)));
}

export function structuralFatalIssues(quality) {
  return (quality?.issues ?? []).filter((entry) => entry?.severity === "fatal" && !isQuarantinableMarketDataIssue(entry));
}

export function createRankingCoverage(snapshot, modelVersion, fallbackCount = 0) {
  const summary = snapshot?.universeSummary ?? {};
  const originalCount = summary.originalUniverse?.count ?? snapshot?.universe?.sourceCount ?? snapshot?.records?.length ?? fallbackCount;
  const ranking = summary.rankingUniverse?.[modelVersion] ?? summary.modelEligibleUniverse?.[modelVersion];
  const rankingCount = ranking?.count ?? fallbackCount;
  return {
    originalUniverseCount: originalCount,
    originalUniverseHash: summary.originalUniverse?.codesHash ?? summary.observedUniverse?.codesHash ?? null,
    qualityEligibleUniverseCount: summary.qualityEligibleUniverse?.count ?? rankingCount,
    qualityEligibleUniverseHash: summary.qualityEligibleUniverse?.codesHash ?? ranking?.codesHash ?? null,
    rankingUniverseCount: rankingCount,
    rankingUniverseHash: ranking?.codesHash ?? null,
    quarantinedCount: summary.quarantinedUniverse?.count ?? 0,
    quarantineHash: summary.quarantinedUniverse?.codesHash ?? null,
    exclusionPolicyVersion: summary.exclusionPolicyVersion ?? null,
    isPartialRanking: summary.isPartialRanking ?? false,
    snapshotReferenceDate: snapshot?.asOfDate ?? null,
  };
}
...
export function createDataQualityMetadata(quality, requestedDate) {
  ...
  return {
    ...
    quarantine: { policyVersion: QUALITY_EXCLUSION_POLICY_VERSION, count: new Set(quarantineExclusions.map((entry) => entry.code)).size, exclusions: quarantineExclusions },
    certification: { eligibleForDisplay: true, eligibleForRanking: structuralFatalIssues(quality).length === 0, eligibleForRankBacktest: false, eligibleForScoreBucketBacktest: false, eligibleForOptimization: quality.eligibleForOptimization },
...
export function assertSnapshotQualityGate(quality) {
  const fatalIssues = structuralFatalIssues(quality);
