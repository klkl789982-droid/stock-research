...
export function validateSnapshot(snapshot, expectedCount = null, options = {}) {
  ...
  if (snapshot.schemaVersion === 6 && snapshot.isPartialRanking) {
    const original = snapshot.universeSummary?.originalUniverse;
    const eligible = snapshot.universeSummary?.qualityEligibleUniverse;
    const quarantined = snapshot.universeSummary?.quarantinedUniverse;
    if (original?.count !== snapshot.records?.length) errors.push("original Universe와 records 수가 일치하지 않습니다.");
    if (!Number.isInteger(eligible?.count) || !Number.isInteger(quarantined?.count) || eligible.count + quarantined.count !== original?.count) errors.push("quality-eligible/quarantined Universe 수가 유효하지 않습니다.");
    if (!snapshot.exclusionPolicyVersion || snapshot.exclusionPolicyVersion !== snapshot.universeSummary?.exclusionPolicyVersion) errors.push("partial ranking exclusion policy가 누락되었습니다.");
  }
  ...
  for (const record of snapshot.records ?? []) {
    ...
    if (record.qualityEligibility?.status === "quarantined") {
      const scoreValues = [...Object.values(record.scores ?? {}), ...Object.values(record.scoresByVersion ?? {})];
      const rankValues = [...Object.values(record.ranks ?? {}), ...Object.values(record.ranksByVersion ?? {})];
      if (record.qualityEligibility.eligible !== false || !Array.isArray(record.qualityEligibility.exclusions) || record.qualityEligibility.exclusions.length === 0) errors.push(`${record.code} quarantine metadata가 유효하지 않습니다.`);
      if (scoreValues.some((value) => value !== null) || rankValues.some((value) => value !== null)) errors.push(`${record.code} quarantine 종목에 점수 또는 순위가 있습니다.`);
    }
  ...
  if (snapshot.schemaVersion === 6 && snapshot.isPartialRanking) {
    const expectedQuarantinedCodes = new Set((snapshot.universeSummary?.quarantinedUniverse?.exclusions ?? []).map((entry) => normalizeStockCode(entry.code)).filter(Boolean));
    const actualQuarantinedCodes = new Set((snapshot.records ?? []).filter((record) => record.qualityEligibility?.status === "quarantined").map((record) => normalizeStockCode(record.code)).filter(Boolean));
    if (expectedQuarantinedCodes.size !== snapshot.universeSummary?.quarantinedUniverse?.count || expectedQuarantinedCodes.size !== actualQuarantinedCodes.size || [...expectedQuarantinedCodes].some((code) => !actualQuarantinedCodes.has(code))) errors.push("quarantine Universe와 records가 일치하지 않습니다.");
  }
