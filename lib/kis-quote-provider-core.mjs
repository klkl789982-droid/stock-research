...
export function parseKisQuote(payload, requestedCode, receivedAt) {
  ...
  const quote={code:requestedCode,source:"KIS",priceBasis:"lastQuotedPrice",price:Number(o.stck_prpr),open:Number(o.stck_oprc),high:Number(o.stck_hgpr),low:Number(o.stck_lwpr),volume:Number(o.acml_vol),change:Number(o.prdy_vrss),rate:Number(o.prdy_ctrt),asOfDate:date(o.stck_bsop_date),asOfTime:time(o.stck_cntg_hour),receivedAt};
  if(![quote.price,quote.open,quote.high,quote.low].every((v)=>Number.isFinite(v)&&v>0)||!Number.isFinite(quote.volume)||quote.volume<0||![quote.change,quote.rate].every(Number.isFinite)) throw new KisQuoteError("KIS_INVALID_QUOTE");
  if(quote.high<Math.max(quote.open,quote.price)||quote.low>Math.min(quote.open,quote.price)||quote.high<quote.low) throw new KisQuoteError("KIS_INVALID_OHLC");
  return quote;
}
