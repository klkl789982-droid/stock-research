...
export function isQuarantinableMarketDataIssue(entry) {
  return entry?.severity === "fatal"
    && entry?.type === "postNonTradingPriceDiscontinuity"
...
export function classifyMarketDataRow(row, olderRow = null, newerRow = null) {
  ...
  if (volume === 0 && ohlMissing) {
    const olderClose = numeric(olderRow?.clpr);
    const newerClose = numeric(newerRow?.clpr);
    if (olderClose !== null && olderClose !== prices.clpr) return { type: "invalidTradingRow", reason: "zeroVolumePriceChanged", prices, volume };
    return {
      type: "nonTradingObservation",
      reason: "tradingHaltOrNoTrade",
      prices,
      volume,
      referenceClose: prices.clpr,
      postNonTradingPriceDiscontinuity: newerClose !== null && newerClose !== prices.clpr,
    };
  }
...
function validateRow(row, code, index, rows, rowIssues) {
  ...
  if (classification.type === "nonTradingObservation") {
    rowIssues.push(issue("warning", "nonTradingObservation", { code, date, rowIndex: index, referenceClose: classification.referenceClose }));
    if (classification.postNonTradingPriceDiscontinuity) rowIssues.push(issue("fatal", "postNonTradingPriceDiscontinuity", { code, date, rowIndex: index, disposition: "quarantine" }));
  }
...
function modelStatus({ validHistory, stale, missing, requestedNonTrading, uniqueTradingDays, exclusionReason = null }) {
  const result = {};
  for (const model of MODEL_VERSIONS) {
    let status = "eligible";
    if (missing) status = "missingData";
    else if (stale) status = "staleLatestDate";
    else if (exclusionReason) status = exclusionReason;
...
export function validateMarketDataQuality(input) {
  ...
  for (const code of [...new Set(universeCodes)].sort()) {
    ...
    const quarantinedIssue = symbolIssues.find(isQuarantinableMarketDataIssue);
    const validHistory = !symbolIssues.some((entry) => entry.severity === "fatal");
    const statuses = modelStatus({ validHistory, stale, missing: false, requestedNonTrading, uniqueTradingDays, exclusionReason: quarantinedIssue?.type ?? null });
  ...
  const quarantinedIssues = fatalIssues.filter(isQuarantinableMarketDataIssue);
  const structuralFatalIssues = fatalIssues.filter((entry) => !isQuarantinableMarketDataIssue(entry));
  const hasStructuralErrors = fatalIssues.length > 0;
  ...
  return {
    ...
    summary: {
      ...
      structuralFatalCount: structuralFatalIssues.length,
      quarantinedCount: new Set(quarantinedIssues.map((entry) => entry.code)).size,
      structuralStatus: structuralFatalIssues.length > 0 ? "failed" : "passed",
      provisionalReasons,
    },
    requirements: {
