...
export function resolveFutureReturns(snapshots, marketPriceLedgers = [], tradingCalendar = { schemaVersion: 1, dates: {} }) {
  ...
  for (const snapshot of ordered) {
    ...
    for (const record of snapshot.records) {
      ...
      const entryPriceRecord = entryLedger?.recordsByCode.get(record.code);
      ...
      if (backtest || entryTarget.status === "resolved") {
        ...
        if (entryTarget.status !== "resolved") {
          ...
          const entryStatus = classifyPrice(entryPriceRecord.openPrice, "missingOpenPrice");
      ...
      for (const horizon of FUTURE_RETURN_HORIZONS) {
        ...
        if (Number.isFinite(existingPrediction)) {
          ...
          const futureRecord = futureLedger?.recordsByCode.get(record.code);
        ...
        const exitPriceRecord = exitLedger.recordsByCode.get(record.code);
        if (!exitPriceRecord) {
        ...
        const exitStatus = classifyPrice(exitPriceRecord.closePrice, "missingExitClosePrice");
...
export function prepareSnapshots(rawSnapshots) {
  return rawSnapshots.map((snapshot) => {
    ...
    return { ...copy, recordsByCode: new Map(copy.records.map((record) => [record.code, record])) };
  });
}
